	...
	<aside
		...
		<div class="sidenav-header">
			<i
				...
				class="ms-1 font-weight-bold text-white"> TopZone ADMIN</span>
			</a>
		</div>
			...
			<ul class="navbar-nav">
				<li class="nav-item"><a
					...
					href="${pageContext.request.contextPath}/admin/index">
						...
						</div> <span class="nav-link-text ms-1">Quản lý điện thoại</span>
				</a></li>
				<li class="nav-item"><a class="nav-link text-white "
					href="${pageContext.request.contextPath}/admin/quanlyloaiDT.html">
						<div
							class="text-white text-center me-2 d-flex align-items-center justify-content-center">
				...
				<nav aria-label="breadcrumb">
					<ol
						...
						<li class="breadcrumb-item text-sm"><a
							class="opacity-5 text-dark" href="javascript:;">TopZone
								Admin</a></li>
						<li class="breadcrumb-item text-sm text-dark active"
							aria-current="page">Quản lý điện thoại</li>
					</ol>
					<h6 class="font-weight-bolder mb-0">Quản lý điện thoại</h6>
				</nav>
				<div class="collapse navbar-collapse mt-sm-0 mt-2 me-md-0 me-sm-4"
					...
					<form action="/admin/index" method="get"
						...
						<div class="input-group input-group-outline">
							<label class="form-label">Nhập tên điện thoại cần tìm</label> <input
		...
		<div class="container-fluid py-4">
			...
			<form method="post" style="width: 600px; margin: auto;"
				...
				<div class="form-group">
					<input name="maDT" value="${DTItem.maDT}" type="hidden"
						class="form-control" readonly
						style="border: inset rgb(112, 112, 112); width: 500px; padding: 10px;">
				</div>
				<div class="form-group">
					<label>Tên DT</label> <input name="tenDT"
						value="${DTItem.tenDT}" type="text" class="form-control"
						style="border: inset rgb(112, 112, 112); width: 500px; padding: 10px;">
				</div>
				<div class="form-group">
					<label>Đơn giá</label> <input name="donGia"
						value="${DTItem.donGia}" type="number" class="form-control"
						style="border: inset rgb(112, 112, 112); width: 500px; padding: 10px;">
				</div>
				<div class="form-group">
					<c:set var="giamGia" value="${DTItem.giamGia*100}" />
				...
				<div class="form-group">
					<label>Loại DT</label> <select name="maLoaiDT"
						class="form-control"
						style="border: inset rgb(112, 112, 112); width: 500px; padding: 10px;">
						<c:forEach var="item" items="${listLoaiDT}">
							<c:choose>
								<c:when test="${item.maLoaiDT==DTItem.loaiDT.maLoaiDT}">
									<option value="${item.maLoaiDT}" selected>${item.tenLoai}</option>
								</c:when>
								<c:otherwise>
									<option value="${item.maLoaiDT}">${item.tenLoai}</option>
								</c:otherwise>
							</c:choose>
						</c:forEach>
					</select>
				...
				<div class="form-group">
					<label>Hãng DT</label> <select name="maHangDT"
						class="form-control"
						style="border: inset rgb(112, 112, 112); width: 500px; padding: 10px;">
						<c:forEach var="item" items="${listHangDT}">
							<c:choose>
								<c:when test="${DTItem.hangDT.maHang==item.maHang}">
				...
				<div class="form-group">
					...
					<textarea name="moTa" class="form-control"
						style="border: inset rgb(112, 112, 112); width: 500px; padding: 10px;"
						rows="5">${DTItem.moTa}</textarea>
				</div>
				<div class="form-group"
					style="margin-bottom: 20px; margin-top: 20px;">
					<label style="margin-right: 20px;">Hình ảnh</label>
					<c:choose>
						<c:when test="${DTItem.hinhAnh !=null}">
							<div>
								<input id="inputImgDT" name="hinhAnh" type="file"
									class="form-control-file"> <input name="hinhAnhTonTai"
									value="${DTItem.hinhAnh}" type="hidden">
							</div>
							<img id="imgDT"
								src="${pageContext.request.contextPath}/images/sp/${DTItem.hinhAnh}"
						...
						<c:otherwise>
							...
							<img hidden="hidden" id="imgDT" src="" width="150px;"
								height="150px;" />
						</c:otherwise>
					</c:choose>
				</div>
				...
				<a class="btn btn-danger"
					href="/admin/index/delete?idDT=${DTItem.maDT}"
					onclick="return confirm('Bạn có muốn xóa ${DTItem.tenDT} không?');"
			...
			<script>
				window.addEventListener('load', function() {
					document.getElementById('inputImgDT').addEventListener(
							'change',
							function() {
								if (this.files && this.files[0]) {
									var img = document
											.getElementById('imgDT');
						...
						<div class="col-lg-6 mb-lg-0 mb-4">
							<div
								...
								, made with <i class="fa fa-heart"></i> by <a
									href="https://www.facebook.com/profile.php?id=100004657267250"
									class="font-weight-bold" target="_blank">PS24040 - PHUONGNTK</a> for a
								better web.
							</div>
						</div>
						<div class="col-lg-6">
